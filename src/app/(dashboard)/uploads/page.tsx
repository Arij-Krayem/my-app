        ...
        {uploads.length === 0 ? (
          <div className="dashboard-empty-state">
            <div className="dashboard-empty-icon">
              <svg width="34" height="34" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round"><path d="M22 19a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5l2 3h9a2 2 0 0 1 2 2z"/></svg>
            </div>
            <div className="dashboard-empty-title">No uploads yet</div>
            <div className="dashboard-empty-subtitle">Bring in your first dataset to start validating mappings, tracking status, and powering the dashboard.</div>
            <div className="dashboard-empty-action">
              <Link href="/uploads/new" className="btn-primary">Upload your first dataset</Link>
            </div>
          </div>
        ) : (
          <>
              ...
              return (
                ...
                >
                  <div style={{ display: "flex", alignItems: "center", gap: "12px", minWidth: 0 }}>
                    ...
                    <span style={{ fontSize: "14px", fontWeight: "600", color: "var(--t1)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{u.fileName}</span>
                  </div>
                  <span style={{ fontSize: "13px", fontWeight: "700", color: u.platform === "GOOGLE" ? "#4285F4" : "#1877F2" }}>{u.platform === "GOOGLE" ? "Google Ads" : "Meta Ads"}</span>
                  <span style={{ fontSize: "13px", color: "var(--t2)" }}>{u.brand || "—"}</span>
                  <span style={{ fontSize: "11px", fontWeight: "800", padding: "4px 10px", borderRadius: "999px", background: sc.bg, color: sc.color, border: `1px solid ${sc.border}`, display: "inline-block", width: "fit-content" }}>{u.status}</span>
